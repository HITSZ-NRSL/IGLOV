...
StereoNode::StereoNode (const ORB_SLAM2::System::eSensor sensor, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) : Node (sensor, node_handle, image_transport) {
    left_sub_ = new message_filters::Subscriber<sensor_msgs::Image> (node_handle, "camera/left/image_raw", 1);
    right_sub_ = new message_filters::Subscriber<sensor_msgs::Image> (node_handle, "camera/right/image_raw", 1);
